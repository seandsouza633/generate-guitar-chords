...
MAJOR_CHORDS = {
    k: v for k, v in CHORDS.items() 
    if k in ["maj", "maj7"]
}

REGULAR_CHORDS = {
    k: v for k, v in CHORDS.items() 
    if k not in ["sus2", "sus4"]
}
...
ALL_STRING_COMBOS = [(i, i + n) for n in range(3, 7) for i in range(7-n)]

def insert_over_x_array(start: int, arr: list):
    ret = [-1 for _ in range(6)]
    ret[start:start] = arr
    ret = ret[:6]
...
def printable_tab(tab) -> str:
    return " ".join([" x" if t == -1 else f"{t:2}" for t in tab])
...
def generate_guitar_tabs(chord, limit=10) -> list[tuple]:
    ...
    for string in GUITAR_STRINGS:
        all_string_positions.append([
                fret for fret in range(0, 12) 
                if note_normal(string + fret) in chord
            ])
    # print(Fore.BLACK + str(all_string_positions))
    # print(Fore.BLACK + f"All string combos: {ALL_STRING_COMBOS}")
    all_fingerings = []
    for start, end in ALL_STRING_COMBOS:
        fingerings = [insert_over_x_array(start, list(p)) for p in itertools.product(*all_string_positions[start:end])]
        all_fingerings.extend(fingerings)
    # print(all_fingerings[:limit])
    all_fingerings = filter( # filtering out tabs where you would have to reach too far
        lambda f: 
            # sum(1 for i in f if i == 0) >= 2 and
            len(list(itertools.combinations(filter(lambda i: i>0, f), 2))) > 0 and
            max([abs(c[0] - c[1]) for c in list(itertools.combinations(filter(lambda i: i>0, f), 2))]) <= MAX_REACH,
        all_fingerings
    )
    all_fingerings = sorted(
        all_fingerings, 
        key=(lambda t: \
                # these weights for the different parts of the eval are arbitrarily chosen
                sum(1 for i in t if i == 0) * 0.5 - \
                statistics.stdev(filter(lambda i: i > 0, t)) + \
                len(list(filter(lambda i: i != -1, t))) * 0.3
            ),
        reverse=True
        )
    # for i, fingering in enumerate(all_fingerings):
    #     if i >= limit: 
    #         break
    #     print(Fore.BLACK + str(fingering))
    return all_fingerings[:limit] if limit < len(all_fingerings) else all_fingerings
...
def main(debug=False):
    ...
    if debug: print(Fore.BLACK + str([NOTES[n] for n in scale]))
    amt = 4
    # amt = 1
    for n in range(amt):
        ...
        if debug: print(Fore.BLACK + f"{chord_name} {chord_positions}")
        base = random.randrange(len(scale))
        # base = 0
        chord = generate_chord(base, scale, chord_positions)
        if debug: print(Fore.BLACK + f"{NOTES[root]}{chord_name:4} {chord}")
        print(Fore.BLUE + f"{NOTES[root]}{chord_name} -> {" ".join([NOTES[note_normal(n)] for n in chord])}")
        tabs = generate_guitar_tabs(chord, limit=3)
